...
- **Transports**: {doc}`IO_HTTP`, {doc}`IO_SQL` (**`fetch_sqlmodel`** / **`iter_sqlmodel`** and async mirrors — SQLModel-first reads), {doc}`IO_EXTRAS`
...
- **Roadmap**: {doc}`ROADMAP`
- **SQLModel-first SQL I/O roadmap (v1.13)**: {doc}`SQLMODEL_SQL_ROADMAP`
