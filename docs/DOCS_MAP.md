...
- **Build an API with FastAPI**: {doc}`GOLDEN_PATH_FASTAPI` (shortest runnable path), then {doc}`FASTAPI` (request bodies, responses, async patterns); roadmap and ergonomics → {doc}`FASTAPI_ENHANCEMENTS`; lazy async scans → {doc}`/cookbook/async_lazy_pipeline`.
