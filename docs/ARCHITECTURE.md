...
Terminal work on a lazy plan can follow **four** materialization modes (blocking, async, deferred **`submit`**, chunked **`stream`** / **`astream`**); see [MATERIALIZATION](/MATERIALIZATION.md).
...
- **Row order is not a stable guarantee** unless explicitly documented; compare on keys when testing (see [INTERFACE_CONTRACT](/INTERFACE_CONTRACT.md)).
