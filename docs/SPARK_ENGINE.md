...
cluster client; it uses the in-process Rust engine. See [PYSPARK_UI](/PYSPARK_UI.md) for that surface. This
...
- The Spark engine stack is **lazy-imported**; `import pydantable` does not load PySpark or
  raikou-core.
- For behaviour guarantees (joins, nulls, windows) on the **default** engine, see [INTERFACE_CONTRACT](/INTERFACE_CONTRACT.md).
