...
Tag the same commit for all distributions: **`v` + version** (for example **`v1.15.2`**). **`pip install pydantable==2.0.0`** resolves **`pydantable-protocol==2.0.0`** and **`pydantable-native==2.0.0`** from PyPI (same pin pattern for future **`2.x.y`** tags).
