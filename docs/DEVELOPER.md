...
Pushing a git tag matching **`v*`** (for example **`v1.14.0`**) runs `.github/workflows/release.yml`: shared CI (same checks as **`ci.yml`**), then publishes **multiple distributions** in order: **`pydantable-protocol`** (sdist + wheel from **`pydantable-protocol/`**), **`pydantable`** (sdist + wheel from the repo root), **`pydantable-native`** (per-platform wheels + one sdist via **`PyO3/maturin-action`** with **`working-directory: pydantable-native`**), and finally **`pydantable-meta`**. The repository needs a **`PYPI_API_TOKEN`** secret (`TWINE_USERNAME` is **`__token__`** in the workflow). **`release.yml`** sets **`permissions: contents: read`** and **`actions: write`** so the reusable **`shared-ci`** job can use **`actions/cache`** and **`actions/upload-artifact`**. Versions come from each package’s **`pyproject.toml`** on the tagged commit. Keep the workflow’s **Python test install** (`.github/workflows/_shared-ci.yml`, **Install maturin and test deps**) aligned with **`pyproject.toml`** **`[project.optional-dependencies]`** **`dev`** + **`pandas`** + **`polars`** + **`pytest-cov`** + **`rapcsv`/`rapfiles`/`rapsqlite`** (plus the other CI-only pins: **`fastapi`**, **`httpx`**, **`sqlalchemy`**, **`streamlit`**, **`dataframe-api-compat`**, **`fsspec`**, **`openpyxl`**, **`kafka-python`**, **`google-cloud-bigquery`**, **`snowflake-connector-python`**, …) so optional tests are not skipped on matrix legs or on tag builds.
