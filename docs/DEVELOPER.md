...
- [ ] **Version matches everywhere (1.x.y):** root `pyproject.toml` (**`pydantable`**), `pydantable-protocol/pyproject.toml`, `pydantable-native/pyproject.toml`, `pydantable-meta/pyproject.toml`, `pydantable-core/Cargo.toml`, `python/pydantable/__init__.py` (**`__version__`**), `pydantable-protocol/python/pydantable_protocol/__init__.py` (**`__version__`**), and pinned deps (`pydantable-protocol==…`, `pydantable==…`, `pydantable-native==…` where applicable). **`rust_version()`** in `pydantable-core/src/python_api/expr_fns.rs` uses `env!("CARGO_PKG_VERSION")`. CI runs `tests/test_version_alignment.py` (`pydantable.__version__ == pydantable_native._core.rust_version()` and protocol matches core).
...
Pushing a git tag matching **`v*`** (for example **`v1.14.0`**) runs `.github/workflows/release.yml`: shared CI (same checks as **`ci.yml`**), then publishes **multiple distributions**: **`pydantable-protocol`**, **`pydantable`**, then **`pydantable-native`** (many parallel **`maturin-action`** jobs — one job uploads the **sdist** plus **manylinux x86_64** wheels). **`pydantable-meta`** runs after **`publish-pypi-manylinux-x86_64`** succeeds (not after every native platform), so one failing optional wheel job does not block the meta upload; remaining native jobs still **`twine upload`** in parallel. The repository needs a **`PYPI_API_TOKEN`** secret (`TWINE_USERNAME` is **`__token__`** in the workflow). **`release.yml`** sets **`permissions: contents: read`** and **`actions: write`** so the reusable **`shared-ci`** job can use **`actions/cache`** and **`actions/upload-artifact`**. Versions come from each package’s **`pyproject.toml`** on the tagged commit. Keep the workflow’s **Python test install** (`.github/workflows/_shared-ci.yml`, **Install maturin and test deps**) aligned with **`pyproject.toml`** **`[project.optional-dependencies]`** **`dev`** + **`pandas`** + **`polars`** + **`pytest-cov`** + **`rapcsv`/`rapfiles`/`rapsqlite`** (plus the other CI-only pins: **`fastapi`**, **`httpx`**, **`sqlalchemy`**, **`streamlit`**, **`dataframe-api-compat`**, **`fsspec`**, **`openpyxl`**, **`kafka-python`**, **`google-cloud-bigquery`**, **`snowflake-connector-python`**, …) so optional tests are not skipped on matrix legs or on tag builds.
...
Tag the same commit for all packages: **`v` + version** (for example **`v1.14.0`**). **`pip install pydantable==1.14.0`** resolves **`pydantable-protocol==1.14.0`** from PyPI; **`pydantable-meta`** pins both **`pydantable`** and **`pydantable-native`** at that version.
