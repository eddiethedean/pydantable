...
Pushing a git tag matching **`v*`** (for example **`v1.14.0`**) runs `.github/workflows/release.yml`: shared CI (same checks as **`ci.yml`**), then publishes **`pydantable-protocol`**, **`pydantable`** (sdist + pure-Python wheel), then **`pydantable-native`** (many parallel **`maturin-action`** jobs; one uploads the **sdist** plus **manylinux x86_64** wheels). The repository needs a **`PYPI_API_TOKEN`** secret (`TWINE_USERNAME` is **`__token__`** in the workflow). **`release.yml`** sets **`permissions: contents: read`** and **`actions: write`** so the reusable **`shared-ci`** job can use **`actions/cache`** and **`actions/upload-artifact`**. Versions come from each package’s **`pyproject.toml`** on the tagged commit. Keep the workflow’s **Python test install** (`.github/workflows/_shared-ci.yml`, **Install maturin and test deps**) aligned with **`pyproject.toml`** **`[project.optional-dependencies]`** **`dev`** + **`pandas`** + **`polars`** + **`pytest-cov`** + **`rapcsv`/`rapfiles`/`rapsqlite`** (plus the other CI-only pins: **`fastapi`**, **`httpx`**, **`sqlalchemy`**, **`streamlit`**, **`dataframe-api-compat`**, **`fsspec`**, **`openpyxl`**, **`kafka-python`**, **`google-cloud-bigquery`**, **`snowflake-connector-python`**, …) so optional tests are not skipped on matrix legs or on tag builds.
...
Tag the same commit for all distributions: **`v` + version** (for example **`v1.14.0`**). **`pip install pydantable==1.14.0`** resolves **`pydantable-protocol==1.14.0`** and **`pydantable-native==1.14.0`** from PyPI.
