...
- **`entei-core`** (separate PyPI package, **`>=0.2.0,<0.3`** via **`pydantable[mongo]`**) — **`MongoRoot`** and columnar materialization; **`EnteiPydantableEngine`** lives in **`pydantable.mongo_entei_engine`**. **`EnteiDataFrame`** / **`EnteiDataFrameModel`** live in **`pydantable.mongo_entei`**, like **`SqlDataFrame`** in **`pydantable.sql_moltres`**. Eager PyMongo column-dict I/O (**`fetch_mongo`**, **`iter_mongo`**, **`write_mongo`**) lives in **`pydantable.io.mongo`** (re-exported from **`pydantable`**) and needs **pymongo** only. User guide: **`MONGO_ENGINE`**. Install **`pip install "pydantable[mongo]"`** or **`pip install "entei-core>=0.2.0,<0.3"`** (lazy engine) with **pymongo** (bundled in **`[mongo]`**). Run **`make test-mongo`** for **`tests/mongo/`** (**mongomock**; covers **entei-core** integration and Mongo I/O). **entei-core**’s own tests ship with that distribution.
