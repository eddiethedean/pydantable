...
- **`entei-core`** (separate PyPI package, **`>=0.2.0,<0.3`** via **`pydantable[mongo]`**) — **`MongoRoot`** and columnar materialization; **`EnteiPydantableEngine`** lives in **`pydantable.mongo_entei_engine`**. **`EnteiDataFrame`** / **`EnteiDataFrameModel`** live in **`pydantable.mongo_entei`**, like **`SqlDataFrame`** in **`pydantable.sql_moltres`**. **`pydantable[mongo]`** also installs **Beanie** (ODM) — **preferred** for app Mongo code: **`pydantable.mongo_beanie.sync_pymongo_collection`** maps a Beanie **`Document`** to a **sync** PyMongo **`Collection`**, and **`EnteiDataFrame.from_beanie`** / **`EnteiDataFrameModel.from_beanie`** are the primary lazy entrypoints. Pydantic **`Schema`** + **`from_collection`** remains supported with a raw **`Collection`**. Eager PyMongo column-dict I/O (**`fetch_mongo`**, **`iter_mongo`**, **`write_mongo`**) lives in **`pydantable.io.mongo`** (re-exported from **`pydantable`**) and needs **pymongo** only. User guide: **`MONGO_ENGINE`**. Install **`pip install "pydantable[mongo]"`** (or **`entei-core`** + **pymongo** + **beanie** with matching pins). Run **`make test-mongo`** for **`tests/mongo/`** (**mongomock**; covers **entei-core** integration and Mongo I/O). **entei-core**’s own tests ship with that distribution.
