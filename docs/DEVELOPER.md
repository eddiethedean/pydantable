...
- **`entei-core`** (separate repository next to this checkout: **`../entei-core`**) — **MongoDB** package with **`EnteiPydantableEngine`**, **`MongoRoot`**, **`EnteiDataFrame`**. Install with **`pip install entei-core`** or **`pip install "pydantable[mongo]"`**. Local dev: **`pip install -e ../entei-core`** so **`tests/mongo/`** and lazy **`pydantable`** imports resolve. **`make test-entei`** runs **`../entei-core/tests`** (**mongomock**).
