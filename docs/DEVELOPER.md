...
**`make native-develop`** runs **`pip install -e ./pydantable-protocol`** (if needed) and **`maturin develop --release`** from **`pydantable-native`**, using **`[tool.maturin] manifest-path = "../pydantable-core/Cargo.toml"`** in **`pydantable-native/pyproject.toml`**. Do **not** install **`pydantable`** itself before the native wheel exists, or **`pip`** will try to download **`pydantable-native==2.0.0-b`** (or the current pin from **`pyproject.toml`**) from PyPI and fail on unreleased pins.
...
- [ ] **Version matches everywhere (`2.x.y` / aligned pins):** root `pyproject.toml` (**`pydantable`**), `pydantable-protocol/pyproject.toml`, `pydantable-native/pyproject.toml`, `pydantable-core/Cargo.toml`, `python/pydantable/__init__.py` (**`__version__`**), `pydantable-protocol/python/pydantable_protocol/__init__.py` (**`__version__`**), and pinned deps (`pydantable-protocol==…`, `pydantable-native==…` on **`pydantable`**). **`rust_version()`** in `pydantable-core/src/python_api/expr_fns.rs` uses `env!("CARGO_PKG_VERSION")`. CI runs **`tests/dataframe/test_version_alignment.py`** (`pydantable.__version__ == pydantable_native._core.rust_version()` and protocol matches core).
...
- [ ] `tests/dataframe/test_version_alignment.py` passes (`__version__ == _core.rust_version()`).
...
Tag the same commit for all distributions: **`v` + version** (for example **`v1.15.2`**). **`pip install pydantable==2.0.0-b`** (**`--pre`** if unpinned) resolves matching **`pydantable-protocol`** and **`pydantable-native`** pins from PyPI (same pin pattern for future **`2.x.y`** tags).
