...
- When row order is not guaranteed, sort by identity keys before comparing (see [INTERFACE_CONTRACT](/INTERFACE_CONTRACT.md)).
- Keep snippets **offline** (no network). For HTTP examples, show structure but don’t require a live URL.

## Cross-linking

- When documenting behavior, link to [INTERFACE_CONTRACT](/INTERFACE_CONTRACT.md).
- When discussing performance/materialization, link to [EXECUTION](/EXECUTION.md).
- When discussing I/O entrypoints, link to [IO_DECISION_TREE](/IO_DECISION_TREE.md).

## Versioning notes

- For 1.x, avoid language like “might change” on stable surfaces unless explicitly marked experimental.
- If a behavior is a hard guarantee, write it once in [INTERFACE_CONTRACT](/INTERFACE_CONTRACT.md) and reference it elsewhere.
