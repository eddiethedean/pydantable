...
See [DEVELOPER](/project/developer/) for contributor setup.
...
**Fix:** `pip install pandas` (not required for the default Polars-style export). Duplicate detection (**`duplicated`**, **`drop_duplicates(keep=False)`**) uses the Rust plan and does **not** need pandas. See [PANDAS_UI](/integrations/alternate-surfaces/pandas-ui/).
...
- See [INTERFACE_CONTRACT](/semantics/interface-contract/) (Introspection) for the contract.
...
- See [INTERFACE_CONTRACT](/semantics/interface-contract/) (Ordering).
...
- Details: [IO_OVERVIEW](/io/overview/) (Engine matrix), [IO_DECISION_TREE](/io/decision-tree/) (Engine selection).
...
- See [EXECUTION](/user-guide/execution/) (Materialization costs).
...
Use [IO_DECISION_TREE](/io/decision-tree/) to pick the right entrypoint.
...
- For **mypy** with `pydantable.mypy_plugin`, transform chains can often be typed automatically; see [TYPING](/user-guide/typing/).
...
Use **`pydantable.testing.fastapi.fastapi_app_with_executor`** / **`fastapi_test_client`** so the app’s lifespan runs and **`RequestContext.executor`** is bound. See [FASTAPI](/integrations/fastapi/fastapi/) (Testing helpers).
