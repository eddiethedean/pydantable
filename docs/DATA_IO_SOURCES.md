...
**Scope:** Polars Rust **0.53.0** (the version pinned by **`pydantable-core`**) compared to the kwargs pydantable forwards from **`pydantable-core/src/plan/execute_polars/scan_kw.rs`** (`dispatch_file_scan`). **Parquet hive / lineage / row index** kwargs landed in {doc}`ROADMAP_1_11_LOCAL_IO` **Phase B1**; **CSV** directory/glob tests and extra **`LazyCsvReader`** kwargs in **Phase B2**; **NDJSON** **`glob`** / **`include_file_paths`** / **`row_index_*`** in **Phase B3**; **IPC** **`IpcScanOptions`** + **`UnifiedScanArgs`** in **Phase B4**; **`read_json`** path/**`scan_kwargs`** semantics (alias of **`read_ndjson`**) in **Phase B5**; **partitioned Parquet writes** (**`partition_by`**) and **`write_*_batches`** path semantics in **Phase D**; remaining gaps are tracked there (e.g. **`ScanArgsParquet.schema`**, **`HiveOptions.schema`**).
