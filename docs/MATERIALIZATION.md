...
Canonical enum: **`PlanMaterialization`** and **`plan_materialization_summary()`** (import from **`pydantable`**). Cost notes: {doc}`EXECUTION`. **FastAPI** route examples for all four modes: {doc}`FASTAPI` (**Four materialization modes**).
...
| **Async (await)** | `PlanMaterialization.ASYNC` (`"async"`) | `await acollect`, `await ato_dict`, `await ato_polars`, `await ato_arrow`, `arows`, `ato_dicts`, … | **`async def`** ASGI handlers; keep the event loop responsive (see {doc}`EXECUTION`) |
...
| **Chunked (dict batches)** | `PlanMaterialization.CHUNKED` (`"chunked"`) | `for b in df.stream(...)`, `async for b in df.astream(...)` | **Streaming HTTP** bodies (e.g. NDJSON lines per batch); still **one** full collect before slicing (see {doc}`EXECUTION`) |
...
Same logical work as blocking, but invoked with **`await`**. When the wheel exposes **`async_execute_plan`**, the engine call is awaited as a Rust coroutine; otherwise work runs in **`asyncio.to_thread`** or your **`executor=`** ({doc}`EXECUTION`).
