...
[PYSPARK_UI](/integrations/alternate-surfaces/pyspark-ui/).
...
**0.19.0:** Matrix **unchanged**—documentation and **0.x** versioning policy only; see [`ROADMAP.md`](/project/roadmap/) **Shipped in 0.19.0**.

**0.20.0:** **`DataFrame.show()`** / **`summary()`** rows above; core discovery helpers are shared with the default **`DataFrame`**. See [`ROADMAP.md`](/project/roadmap/) **Shipped in 0.20.0**.
...
- `rangeBetween`: supported for `window_sum`, `window_avg`, `window_min`, and `window_max` with **multi-column** `orderBy`: range offsets use the **first** key only; see [`WINDOW_SQL_SEMANTICS.md`](/semantics/window-sql-semantics/).
