...
from a **thread-pool future** (see {doc}`/EXECUTION`).
...
Add **`RequestIdMiddleware`** from {doc}`/cookbook/fastapi_observability` if you want **`request_id`**
...
- Cancelling **`await acollect()`** / **`result()`** does **not** cancel in-flight Rust work—see {doc}`/EXECUTION`.
...
- {doc}`/cookbook/fastapi_observability` — request IDs for correlating background logs
