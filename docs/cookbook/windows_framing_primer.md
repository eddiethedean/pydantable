...
See {doc}`/WINDOW_SQL_SEMANTICS` for the detailed rules, including multi-key `rangeBetween`
axis behavior.
...
- Prefer the named window APIs (`Window.partitionBy(...).orderBy(...)`) over generic `Expr.over(...)` (see {doc}`/INTERFACE_CONTRACT`).
