...
See [WINDOW_SQL_SEMANTICS](/WINDOW_SQL_SEMANTICS.md) for the detailed rules, including multi-key `rangeBetween`
axis behavior.
...
- Prefer the named window APIs (`Window.partitionBy(...).orderBy(...)`) over generic `Expr.over(...)` (see [INTERFACE_CONTRACT](/INTERFACE_CONTRACT.md)).
