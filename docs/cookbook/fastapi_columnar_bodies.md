...
See [FASTAPI](/FASTAPI.md) **Columnar OpenAPI and Depends** and **`tests/test_pydantable_fastapi_columnar.py`**.
...
- **Validation cost** depends on `trusted_mode` (see [DATAFRAMEMODEL](/DATAFRAMEMODEL.md)).
- **Nested** row fields become **`list[NestedModel]`** in columnar JSON (see [FASTAPI](/FASTAPI.md) **Columnar OpenAPI and Depends**).
