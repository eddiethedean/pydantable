...
- **Row order is not guaranteed** for many operations; compare using sorted keys (see [INTERFACE_CONTRACT](/semantics/interface-contract/)).
- **All-null groups**: `sum/mean/min/max/...` yield `None`; `count` yields `0` (see [INTERFACE_CONTRACT](/semantics/interface-contract/)).
