...
PlanFrame **1.2.x** fixed **per-step input schema** during `execute_plan` ([planframe#103](https://github.com/eddiethedean/planframe/issues/103)); **1.3.0** adds **`CompileExprContext.resolve_backend_dtype`** and **`resolve_backend_dtype_from_frame`** so adapters can recover dtypes from the live backend frame ([planframe#113](https://github.com/eddiethedean/planframe/issues/113)). Earlier releases also added **`resolve_dtype`** / **`CompileExprContext`** ([planframe#104](https://github.com/eddiethedean/planframe/issues/104)), **`Expr`** stub operators ([planframe#106](https://github.com/eddiethedean/planframe/issues/106)), and **`planframe.materialize`** ([planframe#107](https://github.com/eddiethedean/planframe/issues/107)). Upstream async aliases (`to_dict_async`, `execute_plan_async`, …) are described in PlanFrame’s adapter docs; pydantable’s async escape hatches for **`DataFrameModel`** remain in {doc}`PLANFRAME_FALLBACKS`.
