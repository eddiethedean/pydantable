...
**Deprecation:** avoid **`acollect(..., as_polars=...)`** and **`collect(..., as_polars=...)`**; they emit **`DeprecationWarning`** and are scheduled for removal in **pydantable 2.1.0**. Prefer **`ato_polars()`** / **`to_polars()`**, or columnar **`to_dict()`** / **`collect(as_lists=True)`**. See {doc}`VERSIONING` (upcoming removals).
