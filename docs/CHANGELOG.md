...
- **Documentation:** [CUSTOM_ENGINE_PACKAGE](/CUSTOM_ENGINE_PACKAGE.md) — guide for authors publishing a separate engine package (dependencies, protocol implementation, wiring, expressions, I/O boundaries, testing, PyPI).
...
- **Internal:** Introduced `pydantable.engine` (`NativePolarsEngine`, `get_default_engine`, `get_expression_runtime`) so execution is routed through a single abstraction; `rust_engine` remains a thin delegating module. See [ADR-engines](/ADR-engines.md) and [DEVELOPER](/DEVELOPER.md).
...
- **Schema bridging (Phase 5):** **`sqlmodel_columns`**, **`DataFrameModel.assert_sqlmodel_compatible`** — **`python/pydantable/io/sqlmodel_schema.py`**; tests **`tests/test_sqlmodel_bridge_phase05.py`**; docs [IO_SQL](/IO_SQL.md), [DATAFRAMEMODEL](/DATAFRAMEMODEL.md), [SQLMODEL_SQL_ROADMAP](/SQLMODEL_SQL_ROADMAP.md).
- **Documentation + examples + testing gate (Phase 6):** SQLModel-first SQLite examples **`docs/examples/io/sql_sqlite_sqlmodel_roundtrip.py`**, **`docs/examples/io/sql_sqlite_sqlmodel_streaming.py`**; [IO_SQL](/IO_SQL.md) sections for raw vs SQLModel-first examples; **`tests/test_doc_io_examples.py`** runs **`sql_sqlite_streaming.py`** and the SQLModel scripts alongside existing **`sql_sqlite_*`** examples.

### Deprecated

- **Legacy string-SQL names (Phase 4):** **`fetch_sql`**, **`iter_sql`**, **`write_sql`**, **`afetch_sql`**, **`aiter_sql`**, **`awrite_sql`**, **`write_sql_batches`**, **`awrite_sql_batches`** — emit **`DeprecationWarning`**; migrate to **`*_raw`** or SQLModel helpers. **`DataFrameModel.write_sql`** / **`awrite_sql`** delegate to the same deprecated **`pydantable.io`** entrypoints. Removal no earlier than **`2.0.0`** ([VERSIONING](/VERSIONING.md)). Tests: **`tests/test_sql_string_deprecation.py`**; default test run filters these warnings in **`pyproject.toml`** for backward-compatible suites.

### Docs

- **README / site index / I/O guides:** align **current release** (**1.13.0**), SQL I/O naming (**`fetch_sqlmodel`**, **`fetch_sql_raw`**, deprecations), and pointers to [IO_SQL](/IO_SQL.md) / [SQLMODEL_SQL_ROADMAP](/SQLMODEL_SQL_ROADMAP.md) across **README**, [index](/index.md), [IO_OVERVIEW](/IO_OVERVIEW.md), [IO_DECISION_TREE](/IO_DECISION_TREE.md), [EXECUTION](/EXECUTION.md), [DATA_IO_SOURCES](/DATA_IO_SOURCES.md), [DOCS_MAP](/DOCS_MAP.md), [POLARS_TRANSFORMATIONS_ROADMAP](/POLARS_TRANSFORMATIONS_ROADMAP.md), [ROADMAP](/ROADMAP.md), and the SQLModel roadmap introduction.
- **SQL I/O:** [IO_SQL](/IO_SQL.md), [SQLMODEL_SQL_ROADMAP](/SQLMODEL_SQL_ROADMAP.md), [VERSIONING](/VERSIONING.md) — SQLModel-first default, **`*_raw`** for explicit string SQL, deprecation policy. Runnable examples: raw **`sql_sqlite_roundtrip.py`** / **`sql_sqlite_streaming.py`** and SQLModel-first **`sql_sqlite_sqlmodel_*.py`** (see [IO_SQL](/IO_SQL.md)).

### Changed

- **Version bump:** Align Python package metadata, Rust crate, and published **`__version__`** to **1.13.0**. (This release includes all SQLModel-first SQL I/O work since **v1.12.0** — Phases 0–6 of [SQLMODEL_SQL_ROADMAP](/SQLMODEL_SQL_ROADMAP.md) — in one minor version.)
...
- **Local I/O (1.11.0) — release narrative:** Directory/glob/hive lazy reads, **`scan_kwargs`** / **`write_kwargs`** allowlists, eager **`iter_*`** / **`materialize_*`** guidance, partitioned Parquet writes, multi-file Parquet **`allow_missing_columns`** and observability — details in this **1.11.0** section; ongoing I/O work in [ROADMAP](/ROADMAP.md). **`pydantable.__version__`** / **`rust_version()`** alignment per [VERSIONING](/VERSIONING.md) (**`tests/test_version_alignment.py`**).
- **Local I/O Phase E (1.11.0):** Multi-file Parquet — **`allow_missing_columns`**, Polars schema union, cast / optional-field patterns — [IO_PARQUET](/IO_PARQUET.md); pointers in [DATA_IO_SOURCES](/DATA_IO_SOURCES.md), [IO_DECISION_TREE](/IO_DECISION_TREE.md), [SUPPORTED_TYPES](/SUPPORTED_TYPES.md), [INTERFACE_CONTRACT](/INTERFACE_CONTRACT.md), [PLAN_AND_PLUGINS](/PLAN_AND_PLUGINS.md). Contributor note: `pydantable-core/.../scan_kw.rs`, [DEVELOPER](/DEVELOPER.md).
- **Writes Phase D:** partitioned **`write_parquet`**, batch-writer file vs directory—[IO_PARQUET](/IO_PARQUET.md), [IO_OVERVIEW](/IO_OVERVIEW.md), [IO_DECISION_TREE](/IO_DECISION_TREE.md), [DATA_IO_SOURCES](/DATA_IO_SOURCES.md), [INTERFACE_CONTRACT](/INTERFACE_CONTRACT.md); example **`docs/examples/io/parquet_partitioned_write.py`**.
- **Eager / batched multi-file clarity (1.11.0 Phase C):** **`materialize_*`** single-file contract; **`iter_*` / `aiter_*`** one path per call and Python-side glob/directory expansion; **`iter_chain_batches`**; bounded-memory notes vs **`iter_concat_batches`** and lazy **`read_*`**—[IO_OVERVIEW](/IO_OVERVIEW.md), [IO_DECISION_TREE](/IO_DECISION_TREE.md), [DATA_IO_SOURCES](/DATA_IO_SOURCES.md), [INTERFACE_CONTRACT](/INTERFACE_CONTRACT.md); example **`docs/examples/io/iter_glob_parquet_batches.py`**.
- **Local I/O audit (1.11.0 Phase A):** Polars **0.53.0** vs pydantable **`scan_kwargs`** matrix, directory/glob/hive notes—{ref}`Polars 0.53 vs pydantable scan audit <local-io-audit>`; multi-file entrypoint table—[IO_DECISION_TREE](/IO_DECISION_TREE.md); **Local lazy file scans**—[INTERFACE_CONTRACT](/INTERFACE_CONTRACT.md); path/glob subsections on [IO_PARQUET](/IO_PARQUET.md), [IO_CSV](/IO_CSV.md), [IO_NDJSON](/IO_NDJSON.md), [IO_IPC](/IO_IPC.md), [IO_JSON](/IO_JSON.md); link from [IO_OVERVIEW](/IO_OVERVIEW.md).
- **Parquet B1:** [DATA_IO_SOURCES](/DATA_IO_SOURCES.md) audit + summary table; [IO_PARQUET](/IO_PARQUET.md); [INTERFACE_CONTRACT](/INTERFACE_CONTRACT.md) (Parquet lineage / hive kwargs).
- **CSV B2:** [DATA_IO_SOURCES](/DATA_IO_SOURCES.md) audit + summary table; [IO_CSV](/IO_CSV.md); [INTERFACE_CONTRACT](/INTERFACE_CONTRACT.md) (CSV **`include_file_paths`** / **`row_index_*`**).
- **NDJSON B3:** [DATA_IO_SOURCES](/DATA_IO_SOURCES.md) audit + summary table; [IO_NDJSON](/IO_NDJSON.md); [INTERFACE_CONTRACT](/INTERFACE_CONTRACT.md) (NDJSON **`glob`** / **`include_file_paths`** / **`row_index_*`**).
- **IPC B4:** [DATA_IO_SOURCES](/DATA_IO_SOURCES.md) audit + summary table; [IO_IPC](/IO_IPC.md); [INTERFACE_CONTRACT](/INTERFACE_CONTRACT.md) (IPC **`scan_kwargs`**).
- **JSON `read_json` B5:** [IO_JSON](/IO_JSON.md); [DATA_IO_SOURCES](/DATA_IO_SOURCES.md) (lazy vs array); **`pydantable.io.read_json`** docstring.

### Changed

- **Versioning:** Python package metadata and Rust crate aligned at **1.11.0** for this release; docs “current release” strings ([index](/index.md), [CHANGELOG](/CHANGELOG.md), [ROADMAP](/ROADMAP.md), [POLARS_TRANSFORMATIONS_ROADMAP](/POLARS_TRANSFORMATIONS_ROADMAP.md)) aligned.
...
- **JSON modeling:** **JSON (RFC 8259) vs column types** in [SUPPORTED_TYPES](/SUPPORTED_TYPES.md) (heterogeneous arrays, arbitrary JSON, link from [IO_JSON](/IO_JSON.md)).
- **I/O:** **Eager `export_json` serialization** in [IO_JSON](/IO_JSON.md); extended **`export_json`** docstring in **`pydantable.io`** (`json.dump` + `default=str`); struct → JSON text pointer (**`struct_json_encode`**).
- **Structs:** **`SUPPORTED_TYPES`** and **`INTERFACE_CONTRACT`** — struct JSON / **`with_fields`** / **`rename_fields`** semantics and row-wise limits.
- **FastAPI:** columnar **map** / nested field notes with links to [SUPPORTED_TYPES](/SUPPORTED_TYPES.md) and [IO_JSON](/IO_JSON.md).
- **Roadmap:** Phase A + B + C JSON/struct work summarized in this **1.10.0** section and [ROADMAP](/ROADMAP.md) (**Shipped in 1.10.0**); **`str_json_decode`** / error semantics in [SUPPORTED_TYPES](/SUPPORTED_TYPES.md) and [INTERFACE_CONTRACT](/INTERFACE_CONTRACT.md); [IO_JSON](/IO_JSON.md) cross-link.
- **Phase D (I/O):** [IO_JSON](/IO_JSON.md) — **`read_json`** vs **`read_ndjson`** vs **`materialize_json`**, large-file / **`streaming`** patterns, NDJSON **`scan_kwargs`** presets; example **`docs/examples/io/large_ndjson_patterns.py`**; cross-links from [DATA_IO_SOURCES](/DATA_IO_SOURCES.md), [EXECUTION](/EXECUTION.md), [IO_NDJSON](/IO_NDJSON.md).
- **Phase E (UX) & 1.10.0 JSON/struct summary:** [SELECTORS](/SELECTORS.md) — **`s.structs()`**, **`unnest`**, **`struct_field`** pipeline; cookbook [json_logs_unnest_export](/cookbook/json_logs_unnest_export.md) (NDJSON → unnest → **`export_json`**); [DOCS_MAP](/DOCS_MAP.md) link. **Release narrative:** JSON ↔ schema matrix and I/O tests; struct expressions (**`struct_json_encode`**, path/rename/with-fields); **`str_json_decode`**; Phase D large-file NDJSON docs; Phase E selectors + cookbook + this page.
...
- **PySpark UI parity:** **`groupBy`** returning **`PySparkGroupedDataFrame`** / **`PySparkGroupedDataFrameModel`** (aggregations stay Spark-flavored), **`sort`**, **`crossJoin`**, frame action **`count()` → int** (via **`global_row_count()`**), **`unionByName`** (optional **`allowMissingColumns`**), **`intersect`** / **`subtract`** / **`exceptAll`** (join-layer semantics; **`exceptAll`** aliases **`subtract`**, not Spark multiset **`EXCEPT ALL`**), **`fillna`** / **`dropna`** / **`.na`**, **`printSchema`**, **`explain`**, **`toPandas`**, and the same methods on **`DataFrameModel`**. See [PYSPARK_UI](/PYSPARK_UI.md), [PYSPARK_PARITY](/PYSPARK_PARITY.md), and [INTERFACE_CONTRACT](/INTERFACE_CONTRACT.md).
...
- **Versioning:** bump to **1.9.0** across Python package metadata, Rust crate, and shipped stubs; docs “current release” strings ([index](/index.md), [ROADMAP](/ROADMAP.md), [POLARS_TRANSFORMATIONS_ROADMAP](/POLARS_TRANSFORMATIONS_ROADMAP.md)) aligned.
- **Docs:** **`describe()`** / **`summary()`**, **`SUPPORTED_TYPES`** temporal helpers, **[INTERFACE_CONTRACT](/INTERFACE_CONTRACT.md)**, **[EXECUTION](/EXECUTION.md)**, **[PYSPARK_PARITY](/PYSPARK_PARITY.md)**, **[PARITY_SCORECARD](/PARITY_SCORECARD.md)**, **[DEVELOPER](/DEVELOPER.md)**, **[PANDAS_UI](/PANDAS_UI.md)**, and **[POLARS_TRANSFORMATIONS_ROADMAP](/POLARS_TRANSFORMATIONS_ROADMAP.md)** updated for **1.9.0** behavior.

## [1.8.0] — 2026-03-31

### Added

- **Selectors:** selector-driven column and rename helpers (see `pydantable.selectors` and [SELECTORS](/SELECTORS.md)).
- **Core DataFrame ergonomics:** `row_count`, `clip`, and `drop_nulls` arguments and convenience behavior aligned with the 1.8 parity push (see [POLARS_PARITY_1_8](/POLARS_PARITY_1_8.md) and [PARITY_SCORECARD](/PARITY_SCORECARD.md)).
- **Joins:** additional join argument parity including `join_nulls` and `maintain_order` (typed contract preserved; see [INTERFACE_CONTRACT](/INTERFACE_CONTRACT.md)).
- **Reshape:** `pivot_longer` / `pivot_wider` and related reshape ergonomics (see [POLARS_WORKFLOWS](/POLARS_WORKFLOWS.md) and [INTERFACE_CONTRACT](/INTERFACE_CONTRACT.md) reshape notes).
...
- **Pandas UI (schema-first):** `duplicated` / `drop_duplicates(keep=False)` backed by engine plan steps where Polars is enabled; typed **`get_dummies`** with cardinality guard; eager **`cut`** / **`qcut`**, **`factorize_column`**, narrow **`ewm(...).mean()`** (may require pandas at runtime); façade **`pivot`** delegating to core. See [PANDAS_UI](/PANDAS_UI.md) and [PARITY_SCORECARD](/PARITY_SCORECARD.md).
...
- **`pydantable.fastapi`:** **`columnar_body_model`**, **`columnar_body_model_from_dataframe_model`**, **`columnar_dependency`**, **`rows_dependency`** — OpenAPI-friendly columnar bodies and **`Depends`** factories for **`DataFrameModel`**; see [FASTAPI](/FASTAPI.md).
...
- **`pydantable.fastapi`** (optional **`pip install 'pydantable[fastapi]'`**): **`executor_lifespan`**, **`get_executor`** (``Depends``), **`register_exception_handlers`** for **`MissingRustExtensionError`** / **`pydantic.ValidationError`**. See [GOLDEN_PATH_FASTAPI](/GOLDEN_PATH_FASTAPI.md) and [FASTAPI](/FASTAPI.md).
...
- **`DataFrame.astream`** / **`DataFrameModel.astream`**: async iteration of column **`dict`** chunks after one engine collect (see [EXECUTION](/EXECUTION.md)).
...
- New [FASTAPI_ENHANCEMENTS](/FASTAPI_ENHANCEMENTS.md) (roadmap + “when to use what” matrix); links from [GOLDEN_PATH_FASTAPI](/GOLDEN_PATH_FASTAPI.md), [FASTAPI](/FASTAPI.md), [DOCS_MAP](/DOCS_MAP.md).
- [FASTAPI_ENHANCEMENTS](/FASTAPI_ENHANCEMENTS.md): production **lifespan** snippet (**`executor_lifespan`**, **`get_executor`**, **`register_exception_handlers`**), NDJSON helper semantics, troubleshooting table (422 vs 503, empty streams, executor tuning); **`tests/test_pydantable_fastapi_integration.py`** covers empty NDJSON, Unicode/null, custom **`media_type`**, **`astream`** batching, and golden-path stream parsing.
- [FASTAPI](/FASTAPI.md) **Columnar OpenAPI and Depends**; [fastapi_columnar_bodies](/cookbook/fastapi_columnar_bodies.md) uses generated models; **`tests/test_pydantable_fastapi_columnar.py`** covers OpenAPI schema, aliases, **`rows_dependency`**, and **`pydantable.testing.fastapi`**.
- [FASTAPI](/FASTAPI.md) / [FASTAPI_ENHANCEMENTS](/FASTAPI_ENHANCEMENTS.md) / cookbook: columnar **422** vs **`ValueError`** (**500**), nested **`list[NestedModel]`**, **`TestClient(raise_server_exceptions=False)`**; expanded **`tests/test_pydantable_fastapi_columnar.py`** (cache, nested routes, length mismatch, **`register_handlers`**).
- [fastapi_observability](/cookbook/fastapi_observability.md), [fastapi_background_tasks](/cookbook/fastapi_background_tasks.md) (end-to-end-style examples); example **`docs/examples/fastapi/service_layout/`** (`UserBatch`, health metadata, **400** on length mismatch); **`tests/test_pydantable_errors.py`**, **`tests/test_pydantable_fastapi_service_layout.py`**, broader columnar / handler tests; [FASTAPI_ENHANCEMENTS](/FASTAPI_ENHANCEMENTS.md) Phases 4–6 and 8 marked shipped where applicable.
- [TYPING](/TYPING.md): expanded **`SupportsLazyAsyncMaterialize`** (when to use vs **`DataFrameModelWithRow`**, runtime **`isinstance`** caveats, examples); [DATAFRAMEMODEL](/DATAFRAMEMODEL.md) cross-link from async lazy I/O.
- New [MATERIALIZATION](/MATERIALIZATION.md) page; [EXECUTION](/EXECUTION.md), [INTERFACE_CONTRACT](/INTERFACE_CONTRACT.md), [DATAFRAMEMODEL](/DATAFRAMEMODEL.md), [DOCS_MAP](/DOCS_MAP.md) cross-links.
- [DATAFRAMEMODEL](/DATAFRAMEMODEL.md) **Three layers** (ASCII diagram + rule of thumb + lazy-shape warning); [async_lazy_pipeline](/cookbook/async_lazy_pipeline.md); [fastapi_async_materialization](/cookbook/fastapi_async_materialization.md) prefers **`collect`** / **`to_dict`**.
- [ROADMAP](/ROADMAP.md), [DATA_IO_SOURCES](/DATA_IO_SOURCES.md), and **`docs/async_ideas/`** aligned with async/submit/stream work where applicable.
- **README**, [index](/index.md), [DOCS_MAP](/DOCS_MAP.md), [GOLDEN_PATH_FASTAPI](/GOLDEN_PATH_FASTAPI.md), [TROUBLESHOOTING](/TROUBLESHOOTING.md): FastAPI helpers, **`pydantable.errors`**, cookbooks, **`service_layout`**, and testing helpers cross-linked; troubleshooting bullets repaired.
...
- **Expr (type-specific):** `list_join`, `list_sort`, and `list_unique` on homogeneous
  ...
  engine; semantics in [SUPPORTED_TYPES](/SUPPORTED_TYPES.md) and [INTERFACE_CONTRACT](/INTERFACE_CONTRACT.md)).
- **Docs / tests:** expanded expression contracts in [SUPPORTED_TYPES](/SUPPORTED_TYPES.md) and
  [TYPING](/TYPING.md); integration coverage in `tests/test_type_specific_expr.py`.

### Removed

- **CI / Release:** CycloneDX SBOM generation and upload jobs (too fragile for default
  automation); generate SBOMs locally if required (see [DEVELOPER](/DEVELOPER.md) **Optional CycloneDX SBOMs**).

## [1.2.0] — 2026-03-28

### Added

- **Column types (see [SUPPORTED_TYPES](/SUPPORTED_TYPES.md)):**
...
- **Docs:** practical notes for **`Expr`** comparisons (IP/WKB operands), [TYPING](/TYPING.md) (1.2 scalars), [DATAFRAMEMODEL](/DATAFRAMEMODEL.md) field list.
...
- **1.0.0** readiness documentation:
  - explicit 1.x semver policy in [VERSIONING](/VERSIONING.md),
  - release gate checklist and security-advisory handling in [DEVELOPER](/DEVELOPER.md),
...
- 1.x patch/minor/major policy is defined in [VERSIONING](/VERSIONING.md).
- Behavioral semantics continue to be defined in [INTERFACE_CONTRACT](/INTERFACE_CONTRACT.md).
...
- **JSON (array of objects):** **`read_json`**, **`materialize_json`**, **`export_json`**, **`aread_json`**, **`amaterialize_json`**, **`aexport_json`** — local lazy scan and eager column dicts (see [IO_JSON](/IO_JSON.md)).
- **`read_parquet_url_ctx` / `aread_parquet_url_ctx`:** context managers that delete the temporary Parquet file when the block exits (see [IO_HTTP](/IO_HTTP.md)).
...
- **Docs:** [IO_DECISION_TREE](/IO_DECISION_TREE.md), [IO_JSON](/IO_JSON.md), [IO_HTTP](/IO_HTTP.md) updates, engine matrix in [IO_OVERVIEW](/IO_OVERVIEW.md), FASTAPI executor guidance; README and manual pages refreshed for **0.23.x** I/O.

### Details

- **Rust:** **`ScanFileRoot`**, **`plan_to_lazyframe`**, internal sink exports for lazy writes; join/groupby/reshape entrypoints work with lazy file roots where implemented (see [EXECUTION](/EXECUTION.md) matrix).
- **Python:** **`read_csv_stdin`** uses **`materialize_csv`** internally.
- **Docs:** [EXECUTION](/EXECUTION.md) memory model and streaming/collect compatibility matrix (**`PYDANTABLE_ENGINE_STREAMING`** reserved); [DATA_IO_SOURCES](/DATA_IO_SOURCES.md), [FASTAPI](/FASTAPI.md), [INTERFACE_CONTRACT](/INTERFACE_CONTRACT.md), [ROADMAP](/ROADMAP.md), [README](/REPOSITORY_README.md).
...
- **UX / discovery:** Core **`DataFrame`** and **`DataFrameModel`** expose **`columns`**, **`shape`**, **`empty`**, **`dtypes`**, **`info()`**, and **`describe()`** for **int**, **float**, **bool**, and **str** columns (one **`to_dict()`** materialization). **`shape[0]`** follows **root-buffer** semantics—see [INTERFACE_CONTRACT](/INTERFACE_CONTRACT.md) **Introspection**, [EXECUTION](/EXECUTION.md).
- **Docs:** [QUICKSTART](/QUICKSTART.md) (five-minute tour), repository **`notebooks/five_minute_tour.ipynb`**, [EXECUTION](/EXECUTION.md) sections on **materialization costs**, **import styles**, **copy-as / interchange**; **naming map** in [PANDAS_UI](/PANDAS_UI.md) / [PYSPARK_UI](/PYSPARK_UI.md).
...
- **PySpark façade:** **`DataFrame.show()`** and **`summary()`** (alias of **`describe()`**). See [PYSPARK_UI](/PYSPARK_UI.md), [PYSPARK_PARITY](/PYSPARK_PARITY.md).
- **Documentation:** [README](/REPOSITORY_README.md), [index](/index.md), [ROADMAP](/ROADMAP.md), [PARITY_SCORECARD](/PARITY_SCORECARD.md), [PANDAS_UI](/PANDAS_UI.md), [DEVELOPER](/DEVELOPER.md).

### Details

- **Repr / HTML:** Multi-line **`DataFrame.__repr__`** and **`_repr_html_`** (card-style HTML; grouped/model banners). See [EXECUTION](/EXECUTION.md), **`tests/test_dataframe_repr.py`**.
- **Tests:** **`tests/test_display_options.py`**, **`tests/test_dataframe_discovery.py`**, **`tests/test_rust_engine_verbose_errors.py`**. See [EXECUTION](/EXECUTION.md), [INTERFACE_CONTRACT](/INTERFACE_CONTRACT.md).
- **Release hygiene:** **`make check-full`**, full **pytest**, **`cargo test --all-features`** per [DEVELOPER](/DEVELOPER.md).

## [0.19.0] — 2026-03-24

### Highlights

- **Pre-1.0 consolidation:** [VERSIONING](/VERSIONING.md) documents **0.x** patch vs minor expectations; [INTERFACE_CONTRACT](/INTERFACE_CONTRACT.md) links there for semver scope while staying the behavioral source of truth.
- **Roadmap to 1.0:** [ROADMAP](/ROADMAP.md) **Shipped in 0.19.0** replaces the planned checklist; **Planned v1.0.0** items that belong on the **1.0.0** tag (full **1.x** semver policy, SBOM, comms) remain explicitly deferred there with rationale below.
- **Parity docs:** [POLARS_TRANSFORMATIONS_ROADMAP](/POLARS_TRANSFORMATIONS_ROADMAP.md), [PARITY_SCORECARD](/PARITY_SCORECARD.md), [PYSPARK_PARITY](/PYSPARK_PARITY.md), [README](/REPOSITORY_README.md), and [index](/index.md) updated for **current release** and **0.19 → 1.0** clarity—no new table methods or PySpark `functions` rows.
- **Performance:** [PERFORMANCE](/PERFORMANCE.md) adds an **0.19.0 validation** note (key scripts spot-checked; no headline number refresh vs **0.18.x** paths).
...
See [ROADMAP](/ROADMAP.md) **Shipped in 0.19.0**. Release hygiene: **`make check-full`**, **`cargo test --all-features`**, **`cargo check --no-default-features`**, full **pytest** before tag; GitHub Actions install deps aligned with [DEVELOPER](/DEVELOPER.md) / **`pyproject.toml`** **`[dev]`**.

**Deferred to v1.0.0 tag (not blocking 0.19.0):** formal **1.x** semver publication, PyPI packaging dry-run narrative, SBOM/supply-chain notes, support matrix as a **1.0.x** commitment, and README/index “1.0 leads” copy—see [ROADMAP](/ROADMAP.md) **Planned v1.0.0**.

## [0.18.0] — 2026-03-22

### Highlights

- **Grouped execution errors:** Polars **`collect()`** failures during **`group_by().agg()`** may include **`(group_by().agg())`** in the **`ValueError`** text (via **`polars_err_ctx`**) so they are identifiable as grouped aggregation runtime errors. See [EXECUTION](/EXECUTION.md).
- **Maps:** **Non-string** map keys (**`dict[int, T]`**, non-UTF-8 Arrow map keys) remain **unsupported** and are **explicitly deferred** for this release ([SUPPORTED_TYPES](/SUPPORTED_TYPES.md), [ROADMAP](/ROADMAP.md) **Later**).
- **Documentation:** Post–**P7** note in [POLARS_TRANSFORMATIONS_ROADMAP](/POLARS_TRANSFORMATIONS_ROADMAP.md) (phases complete; further parity is additive). [PARITY_SCORECARD](/PARITY_SCORECARD.md), [PYSPARK_PARITY](/PYSPARK_PARITY.md), [DEVELOPER](/DEVELOPER.md), [ROADMAP](/ROADMAP.md) updated. **No** new PySpark **`sql.functions`** wrappers or table API changes.
...
See [ROADMAP](/ROADMAP.md) **Shipped in 0.18.0**. [INTERFACE_CONTRACT](/INTERFACE_CONTRACT.md) aggregation rules are unchanged; the doc notes optional **`group_by().agg()`** error-message context.

## [0.17.0] — 2026-03-18

### Highlights

- **Maps (string keys):** Documented and tested **Expr** behavior for **`map_get`** / **`map_contains_key`** on columns ingested from PyArrow **`map<utf8, …>`** (missing key → null). **Non-string** Python **`dict[int, T]`** map keys remain **unsupported** (deferred); see [ROADMAP](/ROADMAP.md) **Later**.
...
See [ROADMAP](/ROADMAP.md) **Shipped in 0.17.0**.
...
- **Docs:** [`EXECUTION.md`](/EXECUTION.md), [`SUPPORTED_TYPES.md`](/SUPPORTED_TYPES.md), [`INTERFACE_CONTRACT.md`](/INTERFACE_CONTRACT.md), [`ROADMAP.md`](/ROADMAP.md), [`README.md`](/REPOSITORY_README.md), [`index.md`](/index.md).

### Details

See [ROADMAP](/ROADMAP.md) **Shipped in 0.16.0**. Sync **`read_parquet` / `read_ipc`** are blocking; use **`asyncio.to_thread`** or an executor from **`async def`** routes for large files if loop latency matters.
...
- **Async materialization:** **`acollect`**, **`ato_dict`**, **`ato_polars`** on **`DataFrame`**; **`DataFrameModel`** adds the same plus **`arows`** and **`ato_dicts`**. Work runs in **`asyncio.to_thread`** or an optional **`executor=`**. See [EXECUTION](/EXECUTION.md), [FASTAPI](/FASTAPI.md).
...
- **Arrow-native maps:** PyArrow **`map<utf8, …>`** arrays (and chunked) ingest for **`dict[str, T]`** columns; convert to Python **`dict`** cells. String keys only; **`strict`** checks scalar value types (nested map values: best-effort). Tests: **`tests/test_pyarrow_map_ingest.py`**. [SUPPORTED_TYPES](/SUPPORTED_TYPES.md) updated.
- **PySpark façade:** **`trim`**, **`abs`**, **`round`**, **`floor`**, **`ceil`** in **`pydantable.pyspark.sql.functions`** (and package **`__all__`**). [PYSPARK_PARITY](/PYSPARK_PARITY.md) updated.
...
See [ROADMAP](/ROADMAP.md) **Shipped in 0.15.0**. Sync **`collect` / `to_dict` / `to_polars`** are unchanged aside from constructor kwargs (drop **`validate_data`**; use **`trusted_mode`**). You may replace manual **`asyncio.to_thread`** wrappers with **`acollect`** / **`ato_*`**.
...
- **Window `orderBy` null placement:** **`nulls_last`** on **`Window.partitionBy(...).orderBy(...)`** (per-column list or bool); framed windows use all keys; unframed Polars **`.over`** uses the first key for **`SortOptions`**. Docs: [WINDOW_SQL_SEMANTICS](/WINDOW_SQL_SEMANTICS.md), [INTERFACE_CONTRACT](/INTERFACE_CONTRACT.md).
- **Trusted `shape_only`:** **`pydantable.DtypeDriftWarning`** when data would fail **`strict`**; env **`PYDANTABLE_SUPPRESS_SHAPE_ONLY_DRIFT_WARNINGS=1`** to silence. See [SUPPORTED_TYPES](/SUPPORTED_TYPES.md).
- **`validate_data` deprecation:** explicit **`validate_data=`** without **`trusted_mode`** raises **`DeprecationWarning`** (removal shipped in **0.15.0**). See [DATAFRAMEMODEL](/DATAFRAMEMODEL.md), [FASTAPI](/FASTAPI.md).
- **PySpark façade:** **`dayofmonth`**, **`lower`**, **`upper`** in **`pydantable.pyspark.sql.functions`**. [PYSPARK_PARITY](/PYSPARK_PARITY.md) updated.
- **FastAPI DX:** **`TestClient`** recipes and OpenAPI notes in [FASTAPI](/FASTAPI.md); **`tests/test_fastapi_recipes.py`**; **`fastapi`** / **`httpx`** in **`[dev]`** and CI.
- **Hypothesis:** extra property test for **`with_columns`** identity; [DEVELOPER](/DEVELOPER.md) documents running property tests.
...
See [ROADMAP](/ROADMAP.md) **Shipped in 0.14.0**.

## [0.13.0] — 2026-03-23

### Highlights

- **Stabilization + combined scope:** [FASTAPI](/FASTAPI.md) — **`trusted_mode`** / **`validate_data`**, column-shaped **`dict[str, list]`** bodies, **sync** materialization and pointers forward to async work (shipped in **0.15.0**); trust-boundary guidance for large / pre-validated tables and **Polars** / **Arrow**; install notes for PyPI wheels vs git builds.
- **Sync-only I/O (at time of 0.13.0):** [EXECUTION](/EXECUTION.md) and [PERFORMANCE](/PERFORMANCE.md) described **blocking** materialization; **async** APIs arrived in **0.15.0** ([EXECUTION](/EXECUTION.md), [FASTAPI](/FASTAPI.md)). Tuning text prefers **`trusted_mode`** alongside **`validate_data`**.
- **Window semantics (docs):** null ordering and **`CURRENT ROW`** / peer framing in [WINDOW_SQL_SEMANTICS](/WINDOW_SQL_SEMANTICS.md) and [INTERFACE_CONTRACT](/INTERFACE_CONTRACT.md); `Window` docstring in `window_spec.py`. (**User-facing `NULLS FIRST` / `LAST`** shipped in **0.14.0**.)
...
- **Performance:** `benchmarks/framed_window_bench.py`, `benchmarks/trusted_polars_ingest_bench.py`; [PERFORMANCE](/PERFORMANCE.md) table and cross-links.
- **Discoverability:** [index](/index.md), [README](/REPOSITORY_README.md) roadmap table, and cross-links across [INTERFACE_CONTRACT](/INTERFACE_CONTRACT.md), [WINDOW_SQL_SEMANTICS](/WINDOW_SQL_SEMANTICS.md), [POLARS_TRANSFORMATIONS_ROADMAP](/POLARS_TRANSFORMATIONS_ROADMAP.md).
...
See [FASTAPI](/FASTAPI.md), [EXECUTION](/EXECUTION.md), [PERFORMANCE](/PERFORMANCE.md), [ROADMAP](/ROADMAP.md), and [INTERFACE_CONTRACT](/INTERFACE_CONTRACT.md).

## [0.12.0] — 2026-03-22

### Highlights

- **Multi-key `rangeBetween`:** aggregate window frames may use multiple `orderBy` columns; sort is lexicographic and **range bounds apply to the first** sort key (PostgreSQL-style). Documented in [WINDOW_SQL_SEMANTICS](/WINDOW_SQL_SEMANTICS.md).
...
- **Contracts and parity docs:** refresh `INTERFACE_CONTRACT`, PySpark UI/scorecard, roadmap; add duplicate-key policy for `map_from_entries` ([SUPPORTED_TYPES](/SUPPORTED_TYPES.md)).
...
See [INTERFACE_CONTRACT](/INTERFACE_CONTRACT.md), [PYSPARK_PARITY](/PYSPARK_PARITY.md), [PARITY_SCORECARD](/PARITY_SCORECARD.md), [WINDOW_SQL_SEMANTICS](/WINDOW_SQL_SEMANTICS.md), and [ROADMAP](/ROADMAP.md).
...
See [INTERFACE_CONTRACT](/INTERFACE_CONTRACT.md), [PYSPARK_PARITY](/PYSPARK_PARITY.md), [SUPPORTED_TYPES](/SUPPORTED_TYPES.md), and [ROADMAP](/ROADMAP.md).
...
See [DATAFRAMEMODEL](/DATAFRAMEMODEL.md), [INTERFACE_CONTRACT](/INTERFACE_CONTRACT.md), [SUPPORTED_TYPES](/SUPPORTED_TYPES.md), [PYSPARK_PARITY](/PYSPARK_PARITY.md), and [ROADMAP](/ROADMAP.md).
...
See [SUPPORTED_TYPES](/SUPPORTED_TYPES.md), [PYSPARK_PARITY](/PYSPARK_PARITY.md), [ROADMAP](/ROADMAP.md), and [INTERFACE_CONTRACT](/INTERFACE_CONTRACT.md).
...
- README feature bullets; [INTERFACE_CONTRACT](/INTERFACE_CONTRACT.md) (global `select`); [POLARS_WORKFLOWS](/POLARS_WORKFLOWS.md) (single-row globals example); [index](/index.md), [EXECUTION](/EXECUTION.md), [PYSPARK_UI](/PYSPARK_UI.md), [PYSPARK_PARITY](/PYSPARK_PARITY.md), [PYSPARK_INTERFACE](/PYSPARK_INTERFACE.md).
...
See [SUPPORTED_TYPES](/SUPPORTED_TYPES.md), [PYSPARK_PARITY](/PYSPARK_PARITY.md), and [ROADMAP](/ROADMAP.md).
...
- **PydanTable** naming and docs alignment with the [ROADMAP](/ROADMAP.md) (0.5.x line).
...
- **Rich column types:** nested Pydantic models (structs), homogeneous `list[T]`, `uuid.UUID`, `decimal.Decimal`, `enum.Enum`, plus `explode`, `unnest`, and extended `Expr` helpers (see [SUPPORTED_TYPES](/SUPPORTED_TYPES.md)).

### Details

For phase history and future direction, see [ROADMAP](/ROADMAP.md) and [POLARS_TRANSFORMATIONS_ROADMAP](/POLARS_TRANSFORMATIONS_ROADMAP.md).
