...
- [x] **Docs:** cross-links and “related documentation” sections in [`WINDOW_SQL_SEMANTICS.md`](/WINDOW_SQL_SEMANTICS.md) and [`INTERFACE_CONTRACT.md`](/INTERFACE_CONTRACT.md); [`README.md`](/REPOSITORY_README.md) and doc site [`index.md`](/index.md) aligned with current behavior.
...
- [x] **Parity and roadmap docs:** Pass on [`POLARS_TRANSFORMATIONS_ROADMAP.md`](/POLARS_TRANSFORMATIONS_ROADMAP.md), [`PARITY_SCORECARD.md`](/PARITY_SCORECARD.md), [`PYSPARK_PARITY.md`](/PYSPARK_PARITY.md), [`README.md`](/REPOSITORY_README.md), and doc site [`index.md`](/index.md)—**current release** and **0.19 → 1.0** narrative updated.
...
- [x] **Quickstart:** [`QUICKSTART.md`](/QUICKSTART.md), [`notebooks/five_minute_tour.ipynb`](../notebooks/five_minute_tour.ipynb), links from [`README.md`](/REPOSITORY_README.md), [`index.md`](/index.md), [`DEVELOPER.md`](/DEVELOPER.md).
...
- [x] **Docs:** [`README.md`](/REPOSITORY_README.md), [`index.md`](/index.md), [`PARITY_SCORECARD.md`](/PARITY_SCORECARD.md), [`PYSPARK_PARITY.md`](/PYSPARK_PARITY.md), [`PANDAS_UI.md`](/PANDAS_UI.md).
...
- [x] **Fallback documentation:** documented **`st.dataframe(df.to_polars())`**, **`st.dataframe(df.to_arrow())`**, **`st.data_editor(df.to_arrow())`** (editing fallback), and **`st.write`** behavior in **`STREAMLIT.md`** and an [EXECUTION](/EXECUTION.md) interchange subsection.
...
**Deferred / not in-tree:** Rust **`sqlx`** drivers (documented SQLAlchemy-first). Engine materialization can use **`pyo3-async-runtimes`** + Tokio (**`async_execute_plan`**); general file/SQL I/O still prefers thread offload unless documented otherwise ([EXECUTION](/EXECUTION.md)).
...
- [x] **PySpark UI:** **`groupBy`** / grouped **`pivot`** / dict-form **`agg`**, **`crossJoin`**, frame **`count()`**, **`unionByName`**, set-style **`intersect`** / **`subtract`** / **`exceptAll`**, **`fillna`** / **`dropna`** / **`.na`**, **`printSchema`**, **`explain`**, **`toPandas`** (and **`DataFrameModel`** parity). See [CHANGELOG](/CHANGELOG.md) **1.9.0**, [PYSPARK_UI](/PYSPARK_UI.md), [PYSPARK_PARITY](/PYSPARK_PARITY.md).
...
- [x] **Struct expressions (Polars):** **`Expr.struct_json_encode`**, **`struct_json_path_match`**, **`struct_rename_fields`**, **`struct_with_fields`**; PySpark **`struct_json_encode`** / **`struct_json_path_match`**. See [CHANGELOG](/CHANGELOG.md) **1.10.0**.
...
- [x] **Docs:** [IO_JSON](/IO_JSON.md), [SUPPORTED_TYPES](/SUPPORTED_TYPES.md), [SELECTORS](/SELECTORS.md), NDJSON cookbook; Phase D/E narrative in [CHANGELOG](/CHANGELOG.md) **1.10.0**.

---

## Shipped in 1.11.0 (local I/O ↔ Polars parity)

- [x] **Multi-file & directory reads:** **`scan_kwargs`** for **Parquet** (hive-style partitions, lineage, row index), **CSV** directory/glob + **`LazyCsvReader`** options, **NDJSON** (**`glob`** and related), **IPC** multi-file, **`read_json`** path semantics — see [CHANGELOG](/CHANGELOG.md) **1.11.0**, [IO_OVERVIEW](/IO_OVERVIEW.md), [DATA_IO_SOURCES](/DATA_IO_SOURCES.md), [IO_DECISION_TREE](/IO_DECISION_TREE.md), {ref}`Polars 0.53 vs pydantable scan audit <local-io-audit>`.
- [x] **Partitioned / dataset writes:** **Parquet** **`partition_by`** hive-style layout; **`write_*_batches`** directory vs single-file semantics — see [CHANGELOG](/CHANGELOG.md) **1.11.0**, [IO_PARQUET](/IO_PARQUET.md).
- [x] **Eager & iterators:** **`materialize_*`** single-file contract; **`iter_*`** patterns and **`iter_chain_batches`** — see [CHANGELOG](/CHANGELOG.md) **1.11.0**, [IO_OVERVIEW](/IO_OVERVIEW.md).
- [x] **Docs & tests:** examples under **`docs/examples/io/`**, [CHANGELOG](/CHANGELOG.md) **1.11.0**, **kwargs** allowlist tests.

**Summary:** [CHANGELOG](/CHANGELOG.md) **1.11.0** documents user-visible local I/O changes.
...
- [x] **Semver contract:** publish a **1.0** policy (expand [`VERSIONING.md`](/VERSIONING.md) and/or [`README.md`](/REPOSITORY_README.md)): what counts as **patch** vs **minor** vs **major** for **1.x** for `DataFrame` / `DataFrameModel` / `Expr` / Rust extension boundaries; confirm [`INTERFACE_CONTRACT.md`](/INTERFACE_CONTRACT.md) is the behavioral source of truth. (**0.x** expectations already live in [`VERSIONING.md`](/VERSIONING.md).)
...
- [x] **Chunked async iterator (`astream`):** **`DataFrame.astream`** / **`DataFrameModel.astream`** yield column **`dict`** chunks after one engine collect (same as **`collect_batches`** semantics — not incremental scan streaming). See [EXECUTION](/EXECUTION.md), [INTERFACE_CONTRACT](/INTERFACE_CONTRACT.md).
...
- [x] **`struct_json_encode`** / **`struct_json_path_match`** symmetry with string JSON
  helpers.
- [x] **`str_json_decode`** (string JSON → struct / **`dict[str, T]`**; see [CHANGELOG](/CHANGELOG.md) **1.10.0**).
