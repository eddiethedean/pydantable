...
End-to-end guide for shipping a third-party engine on PyPI: {doc}`CUSTOM_ENGINE_PACKAGE`. In-repo optional integrations: {doc}`MOLTRES_SQL` (**moltres-core**), {doc}`MONGO_ENGINE` (**entei-core** for **`EnteiDataFrame`**). Eager Mongo column-dict **`fetch_mongo`** / **`write_mongo`** (**PyMongo** only) is normal I/O, not an **`ExecutionEngine`** swap-in.
