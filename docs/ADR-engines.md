...
End-to-end guide for shipping a third-party engine on PyPI: [CUSTOM_ENGINE_PACKAGE](/CUSTOM_ENGINE_PACKAGE.md). In-repo optional integrations: [SQL_ENGINE](/SQL_ENGINE.md) (lazy-SQL stack), [MONGO_ENGINE](/MONGO_ENGINE.md) (**`MongoPydantableEngine`** in **pydantable**, **`MongoRoot`** from the Mongo plan stack for **`MongoDataFrame`**). Eager Mongo column-dict helpers (**`fetch_mongo`** / **`write_mongo`**, **`afetch_mongo`** / **`awrite_mongo`**, … — **PyMongo**) are normal I/O, not a third-party **`ExecutionEngine`** package.
