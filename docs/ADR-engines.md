...
5. **`pydantable-native`** depends only on **`pydantable-protocol`** (not **`pydantable`**). **`pydantable-meta`** / application installs still combine **`pydantable`** + **`pydantable-native`** for the full stack.
...
End-to-end guide for shipping a third-party engine on PyPI: {doc}`CUSTOM_ENGINE_PACKAGE`.
