...
End-to-end guide for shipping a third-party engine on PyPI: {doc}`CUSTOM_ENGINE_PACKAGE`. In-repo optional integrations: {doc}`MOLTRES_SQL` (**moltres-core**), {doc}`MONGO_ENGINE` (**`EnteiPydantableEngine`** in **pydantable**, **entei-core** **`MongoRoot`** for **`EnteiDataFrame`**). Eager Mongo column-dict **`fetch_mongo`** / **`write_mongo`** (**PyMongo** only) is normal I/O, not a third-party **`ExecutionEngine`** package.
