...
| **Lazy typed transforms on a MongoDB collection** (swap-in engine) | **Preferred:** **`pydantable[mongo]`** — Beanie **`Document`** + **`EnteiDataFrame[Schema].from_beanie(Doc, database=sync_db)`** ({doc}`MONGO_ENGINE`). **Also:** **`from_collection(coll)`** with a sync **`Collection`**. | Lazy **`DataFrame`** / **`DataFrameModel`** |
| **Eager MongoDB column dict** (read / write **`dict[str, list]`**, no **`DataFrame`**) | **`fetch_mongo(sync_pymongo_collection(Doc, db), …)`** or **`fetch_mongo(collection, …)`**; **`write_mongo`** ({doc}`MONGO_ENGINE`); **`pydantable[mongo]`** includes **pymongo** | Column dict batches or insert count |
