...
| **Lazy typed transforms on a MongoDB collection** (swap-in engine) | **`pydantable[mongo]`** — **`EnteiDataFrame[Schema].from_collection(coll)`** / **`EnteiDataFrameModel.from_collection(coll)`** ({doc}`MONGO_ENGINE`); **PyMongo** **`Collection`** | Lazy **`DataFrame`** / **`DataFrameModel`** |
| **Eager MongoDB column dict** (read / write **`dict[str, list]`**, no **`DataFrame`**) | **`fetch_mongo`** / **`iter_mongo`** / **`write_mongo`** and **`afetch_mongo`** / **`aiter_mongo`** / **`awrite_mongo`** ({doc}`MONGO_ENGINE`); **`pydantable[mongo]`** includes **pymongo** | Column dict batches or insert count |
