...
| **Lazy typed transforms on SQL** (plans compiled to SQL; swap-in engine) | **`SqlDataFrame`** / **`SqlDataFrameModel`** with **`sql_config=`** / **`sql_engine=`** — install **`pydantable[sql]`** ([SQL_ENGINE](/SQL_ENGINE.md)) | Lazy **`DataFrame`** / **`DataFrameModel`** |
