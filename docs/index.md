...
```{note}
This **documentation site** is the detailed manual. The repository **README** on your Git host is the short entrypoint; both should stay aligned for install commands and version. **Current release:** **1.5.0** (see {doc}`changelog`) — stable **1.x** API under the policy in {doc}`VERSIONING`. Roadmap history and the completed **v1.0.0** gate checklist live in {doc}`ROADMAP`.
```

## Choose your path

- **Services (FastAPI)**: {doc}`GOLDEN_PATH_FASTAPI` (shortest runnable path), then {doc}`FASTAPI`, {doc}`DATAFRAMEMODEL`, and {doc}`EXECUTION`; async lazy reads → {doc}`/cookbook/async_lazy_pipeline`.
