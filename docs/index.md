...
- **Third-party execution backends**: {doc}`CUSTOM_ENGINE_PACKAGE` (ship your own `ExecutionEngine` package with `pydantable-protocol`). Optional Moltres SQL engine: {doc}`MOLTRES_SQL` (**`SqlDataFrame`** / **`SqlDataFrameModel`**, **`pydantable[moltres]`**). Optional Mongo: {doc}`MONGO_ENGINE` — **`pydantable[mongo]`** includes **Beanie**; **prefer** [Beanie](https://github.com/BeanieODM/beanie) **`Document`** (**`from_beanie`**, **`sync_pymongo_collection`**); Pydantic **`Schema`** + **`from_collection`** is supported but not the primary path. Eager **`fetch_mongo`** / **`write_mongo`** use **PyMongo** sync **`Collection`**.
