...
```{note}
This **documentation site** is the detailed manual. The repository **README** on your Git host is the short entrypoint; both should stay aligned for install commands and version. **Current release:** **1.0.0** (see {doc}`changelog`). The project is tracking **Planned v1.0.0** as a stability/semver milestone in {doc}`ROADMAP`.
```
