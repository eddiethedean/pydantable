...
- **`iter_csv`**, **`aiter_csv`**, **`write_csv_batches`** — stdlib **`csv`** batching over paths or text streams; cell values are **strings** (or **`None`** for short rows). See [IO_OVERVIEW](/IO_OVERVIEW.md) (**Batched column dict I/O**).

**`scan_kwargs`:** for example **`separator`**, **`has_header`**, **`skip_rows`**, **`skip_lines`**, **`n_rows`**, **`infer_schema_length`**, **`ignore_errors`**, **`low_memory`**, **`rechunk`**, **`glob`**, **`cache`**, **`quote_char`**, **`eol_char`**, **`include_file_paths`**, **`row_index_name`**, **`row_index_offset`**, **`raise_if_empty`**, **`truncate_ragged_lines`**, **`decimal_comma`**, **`try_parse_dates`**. Unknown keys raise **`ValueError`**. See [DATA_IO_SOURCES](/DATA_IO_SOURCES.md).
...
**`write_kwargs`:** **`include_header`**, **`include_bom`**. See [DATA_IO_SOURCES](/DATA_IO_SOURCES.md).
