...
**Optional engines (1.17.0+):** you can swap **`ExecutionEngine`** implementations while keeping the **`DataFrame`** / **`DataFrameModel`** API — SQL plans via **`pydantable[moltres]`** ({doc}`MOLTRES_SQL`), Mongo collection-backed frames via **`pydantable[mongo]`** ({doc}`MONGO_ENGINE`: **`EnteiPydantableEngine`** subclasses **`NativePolarsEngine`**; **entei-core** supplies **`MongoRoot`** / materialization only). Physical execution remains the **native** Rust core; **moltres-core** affects SQL compilation, not Mongo. Eager Mongo **`fetch_mongo`** / **`write_mongo`** does **not** use **`DataFrame._engine`** — it is column-dict I/O like **`fetch_sqlmodel`**.
