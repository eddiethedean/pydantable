...
**File / I/O:** use **`DataFrameModel`** / **`DataFrame[Schema]`** for lazy **`read_*`** / **`aread_*`** and SQL **`write_sql`** / **`awrite_sql`**. Eager **`materialize_*`**, **`fetch_sql`**, **`iter_sql`**, … live on **`pydantable.io`** — pass **`dict[str, list]`** into **`MyModel(...)`** for typed frames. Low-level **`pydantable.io`** also returns **`ScanFileRoot`** for lazy scans — see {doc}`IO_OVERVIEW`. **Which entrypoint?** {doc}`IO_DECISION_TREE`.
...
| File round-trip | **`pydantable.io.materialize_*`** + **`MyModel(cols)`** / **`export_*`**; or **`read_*`** + transforms + **`write_parquet`** | **`[arrow]`** (buffers); **`[polars]`** (Rust export + lazy **`write_*`** path); core wheel includes Rust readers |
