...
**PlanFrame 1.2.x–1.3.x** tightens the story pydantable relies on: per-step schema in `execute_plan` ([planframe#103](https://github.com/eddiethedean/planframe/issues/103)), `resolve_dtype` / `CompileExprContext` ([planframe#104](https://github.com/eddiethedean/planframe/issues/104)), **`resolve_backend_dtype` / `resolve_backend_dtype_from_frame`** in **1.3.0** ([planframe#113](https://github.com/eddiethedean/planframe/issues/113)), **`Expr` operator overloads** for comparisons and booleans ([planframe#106](https://github.com/eddiethedean/planframe/issues/106)), and **`planframe.materialize`** ([planframe#107](https://github.com/eddiethedean/planframe/issues/107)). pydantable pins **`planframe>=1.3.0,<2`** and documents the Phase T3 bridge in {doc}`TYPING`.
