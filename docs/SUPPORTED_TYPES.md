...
**0.16.0 — PyArrow `Table` / `RecordBatch`:** When **`pyarrow`** is installed, you may pass a **`pa.Table`** or **`RecordBatch`** as **`DataFrame` / `DataFrameModel`** input. It is converted to **`dict[str, list]`** via **`to_pylist()`** per column (copies), then the usual validation runs. **`pydantable.io.materialize_parquet`** / **`materialize_ipc`** produce the same column shape for file/bytes sources. **`DataFrame.to_arrow()`** goes the other way: execute the plan as for **`to_dict()`**, then **`pyarrow.Table.from_pydict`**. Supported cell types are those that already round-trip through list materialization (scalars, **JSON-friendly** nested shapes per engine limits); exotic Arrow extension types may not map cleanly—validate with **`trusted_mode='off'`** when in doubt.
