...
- **`str_json_decode`**: supply a **nested model type** or **`dict[str, T]`** as **`dtype`** (same style as **`cast`**). Prefer pairing with **`struct_json_encode`** for round trips on struct columns. **Map** values in JSON must use Polars’ physical encoding: a JSON **array** of objects with **`key`** / **`value`** fields (not a single JSON object with arbitrary keys). Invalid JSON handling differs from **`str_json_path_match`**—see {doc}`INTERFACE_CONTRACT`.
