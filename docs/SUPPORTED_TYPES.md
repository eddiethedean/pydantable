...
When data is **JSON on the wire** (files, HTTP bodies), use this mapping from JSON value kinds to pydantable fields. For **lazy** JSON Lines scans vs **eager** array files, see [IO_JSON](/IO_JSON.md).
...
See [CUSTOM_DTYPES](/CUSTOM_DTYPES.md) for the full guide.
...
see [INTERFACE_CONTRACT](/INTERFACE_CONTRACT.md) row-wise notes).
...
**0.18.0 / 0.19.0 / 0.20.0 — Non-string map keys:** **`dict[int, T]`**, other non-string Python keys, and Arrow **`map`** types whose keys are not **`string` / `large_string`** remain **unsupported**. That work is explicitly deferred; see [ROADMAP](/ROADMAP.md) **Later**. **0.20.0** does not change map dtypes or ingest.
...
- **`str_json_decode`**: supply a **nested model type** or **`dict[str, T]`** as **`dtype`** (same style as **`cast`**). Prefer pairing with **`struct_json_encode`** for round trips on struct columns. **Map** values in JSON must use Polars’ physical encoding: a JSON **array** of objects with **`key`** / **`value`** fields (not a single JSON object with arbitrary keys). Invalid JSON handling differs from **`str_json_path_match`**—see [INTERFACE_CONTRACT](/INTERFACE_CONTRACT.md).
...
or a **Polars `DataFrame`** as documented in [EXECUTION](/EXECUTION.md) and [PERFORMANCE](/PERFORMANCE.md);
...
See [DATAFRAMEMODEL](/DATAFRAMEMODEL.md) (“Trusted ingest”). The legacy **`validate_data`** constructor argument was removed in **0.15.0**.
...
- [DATAFRAMEMODEL](/DATAFRAMEMODEL.md) — `DataFrameModel` and row vs column inputs
- [INTERFACE_CONTRACT](/INTERFACE_CONTRACT.md) — null semantics, joins, reshape constraints
