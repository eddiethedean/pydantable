...
- Update docs to recommend **SQLModel-first** and describe **raw SQL** as the advanced / migration path — see [IO_SQL](/io/sql/), [DATAFRAMEMODEL](/user-guide/dataframemodel/), and this page.
...
- Explicit **`*_raw`** naming for string-SQL I/O (see [IO_SQL](/io/sql/)).
...
- Legacy names remain until a **major** release (no earlier than **`2.0.0`**); see [VERSIONING](/semantics/versioning/).
...
**Status (v1.13.0): Implemented** — `python/pydantable/io/sqlmodel_schema.py`, `DataFrameModel.assert_sqlmodel_compatible` in `python/pydantable/dataframe_model.py`; tests `tests/test_sqlmodel_bridge_phase05.py`; docs [IO_SQL](/io/sql/), [DATAFRAMEMODEL](/user-guide/dataframemodel/).
...
- Update [IO_SQL](/io/sql/) to reflect SQLModel-first APIs and the `replace_ok` safety knob.
