...
| **L** (Liskov substitution) | Any `ExecutionEngine` implementation (native, stub, SQL bridge) must honor the protocol: same error types for unsupported ops, capabilities that match behavior. |
...
- **Mongo plan stack** (PyPI package pinned **`>=0.2.0,<0.3`** via **`pydantable[mongo]`**) — **`MongoRoot`** and columnar materialization; **`MongoPydantableEngine`** lives in **`pydantable.mongo_dataframe_engine`**. **`MongoDataFrame`** / **`MongoDataFrameModel`** live in **`pydantable.mongo_dataframe`**, like **`SqlDataFrame`** in **`pydantable.sql_dataframe`**. **`pydantable[mongo]`** also installs **Beanie** (ODM) — **preferred** for app Mongo code: **`pydantable.mongo_beanie.sync_pymongo_collection`** maps a Beanie **`Document`** to a **sync** PyMongo **`Collection`**, and **`MongoDataFrame.from_beanie`** / **`from_beanie_async`** / **`MongoDataFrameModel.from_beanie`** / **`from_beanie_async`** are the lazy entrypoints. Pydantic **`Schema`** + **`from_collection`** remains supported with a raw **`Collection`**. Eager PyMongo column-dict I/O (**`fetch_mongo`**, **`iter_mongo`**, **`write_mongo`**, **`afetch_mongo`**, **`aiter_mongo`**, **`awrite_mongo`**) lives in **`pydantable.io.mongo`** (re-exported from **`pydantable`**) and needs **pymongo** only. User guide: **`MONGO_ENGINE`**. Install **`pip install "pydantable[mongo]"`**. Run **`make test-mongo`** for **`tests/mongo/`** (**mongomock**; covers Mongo plan integration and Mongo I/O). The plan package’s own tests ship with that distribution.
...
- `docs/`: product docs + roadmap/spec docs (built with **MkDocs** + Material; see below)
...
**Read the Docs** (`.readthedocs.yaml`) builds with **MkDocs** and **Material for MkDocs** (`mkdocs.yml` at the repository root). RTD installs **MkDocs + Material + mkdocstrings + pymdown-extensions** plus the same lightweight typing stack as before (**SQLAlchemy**, **Mongo** / **Beanie**, …) so API pages resolve — it does **not** run **`pip install -e .[docs]`** with **maturin**, because compiling the native extension typically **exceeds** RTD limits. `docs/mkdocs_hooks.py` adds `python/` and `pydantable-protocol/python/` to `sys.path` and applies the same SQLAlchemy **`Engine`** / **`Connection`** patch as the old Sphinx `conf.py`, so imports work without `pydantable-native`.
...
Strict mode (warnings as errors), matching CI:
...
4. **Docs / changelog:** user-visible behavior belongs in the docs site and `docs/project/changelog.md`.
...
Use this table to locate **Python tests and doc-example smoke** that back shipped minors (see `docs/project/changelog.md` for narrative highlights). It is not a substitute for line coverage.
...
- **Grouped execution error context:** Rust `polars_err_ctx` on **`group_by().agg()`** `collect()` in `execute_polars/common.rs` / `groupby_exec.rs`; notes in `docs/user-guide/execution.md` and `docs/semantics/interface-contract.md`. Unit tests: `polars_err_format_tests` in `execute_polars/common.rs`.
- **Non-string map keys:** deferred—`docs/user-guide/supported-types.md`, `docs/project/roadmap.md` **Later** (no new ingest code).
...
- **`lifespan`** + **`ThreadPoolExecutor`:** covered by **`tests/test_fastapi_recipes.py`** **`test_lifespan_thread_pool_executor_acollect`** (mirrors **`docs/integrations/fastapi/fastapi.md`** async lifespan snippet).
...
See [Documentation builds (MkDocs + Material)](#docs-mkdocs-build) for `mkdocs build --strict`.
...
- **Local I/O kwargs:** New `scan_kwargs` / `write_kwargs` keys belong in `pydantable-core/src/plan/execute_polars/scan_kw.rs` (`ALLOWED` slices) and in `docs/io/data-io-sources.md` (and the format page) so unknown-key errors match what users can pass.
...
- [ ] `docs/project/changelog.md` has a section for the release with highlights
...
- [ ] `mkdocs build --strict` succeeds (matches CI / Read the Docs)
...
Tag the same commit for all distributions: **`v` + version** (for example **`v1.18.0`**). **`pip install pydantable==1.18.0`** resolves **`pydantable-protocol==1.18.0`** and **`pydantable-native==1.18.0`** from PyPI.
