...
This document snapshots **statement-line gaps** by area and complements [Testing](/project/testing.md). **`mypy_plugin.py`** is omitted from measurement ([`pyproject.toml`](../pyproject.toml) `[tool.coverage.run] omit`).
