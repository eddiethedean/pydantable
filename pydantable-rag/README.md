...
- The **Dockerfile** sets **`RAG_PRELOAD_MODELS_ON_STARTUP=true`**, **`RAG_BLOCKING_STARTUP_WARMUP=false`** (non-blocking so health checks pass while the model downloads), **`WEB_CONCURRENCY=1`**, **`HF_HOME=/tmp/hf`**, and **uvicorn `--workers 1`**. Warm-up runs in a **background thread**; **`POST /chat`** still **awaits** **`warm_llm`** until the model is ready. Set **`RAG_BLOCKING_STARTUP_WARMUP=true`** only if your platform allows a long startup without killing the container. **`RAG_AUTO_INGEST_ON_STARTUP`** defaults **off** in code; ship the **CI-built DB** so the image includes **`data/pydantable_vectors.db`**. **`POST /bootstrap`** skips re-ingestion when the index already has rows. Set **`HF_TOKEN`** / **`HUGGING_FACE_HUB_TOKEN`** on the host for Hugging Face. Use **`GET /diag`** (`llm_last_error`) if the LLM fails to load.
