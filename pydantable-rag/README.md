...
- The **Dockerfile** sets **`RAG_PRELOAD_MODELS_ON_STARTUP=true`**, **`RAG_BLOCKING_STARTUP_WARMUP=true`**, and runs **uvicorn with `--workers 1`** so the LLM finishes loading in the same process before traffic is served (in-memory load state is not split across workers). Override blocking preload if your platform’s health probe times out during the first Hugging Face download. **`RAG_AUTO_INGEST_ON_STARTUP`** defaults **off** in code; ship the **CI-built DB** so the image includes **`data/pydantable_vectors.db`**. **`POST /bootstrap`** skips re-ingestion when the index already has rows. **`POST /chat`** waits for **`warm_llm`** instead of returning 503 and relying on a background task. Set **`HF_TOKEN`** / **`HUGGING_FACE_HUB_TOKEN`** on the host for Hugging Face. Use **`GET /diag`** (`llm_last_error`) if the LLM fails to load.
