...
- The **Dockerfile** sets **`RAG_PRELOAD_MODELS_ON_STARTUP=false`** and **`RAG_WARM_LLM_WHEN_INDEX_READY=false`** so new replicas **do not** immediately download/load the LLM (that pattern **OOMs** small instances and causes **crash loops** in the logs). Use **`POST /bootstrap`** after deploy (or set **`RAG_PRELOAD_MODELS_ON_STARTUP=true`** *and* **`RAG_WARM_LLM_WHEN_INDEX_READY=true`** in the dashboard when you run **one replica**, **`HF_TOKEN`** is set, and memory is sufficient). **`RAG_BLOCKING_STARTUP_WARMUP=false`**, **`WEB_CONCURRENCY=1`**, **`HF_HOME=/tmp/hf`**, **uvicorn `--workers 1`**. **`POST /chat`** returns **503** quickly if the LLM is not ready; poll **`GET /readyz`** until **`ok: true`**. **`RAG_AUTO_INGEST_ON_STARTUP`** defaults **off** in code; ship the **CI-built DB**. **`POST /bootstrap`** skips re-ingestion when the index already has rows. Set **`HF_TOKEN`** / **`HUGGING_FACE_HUB_TOKEN`** for Hugging Face. Use **`GET /diag`** (`llm_last_error`) if the LLM fails to load.
