...
- The **Dockerfile** sets **`RAG_PRELOAD_MODELS_ON_STARTUP=true`**, **`RAG_BLOCKING_STARTUP_WARMUP=false`** (non-blocking so health checks pass while the model downloads), **`WEB_CONCURRENCY=1`**, **`HF_HOME=/tmp/hf`**, and **uvicorn `--workers 1`**. **`RAG_WARM_LLM_WHEN_INDEX_READY`** defaults **true** in code: if the vector DB already has rows, the LLM warm-up runs even when a hosting dashboard sets **`RAG_PRELOAD_MODELS_ON_STARTUP=false`** (which would otherwise skip loading). Warm-up runs in a **background thread**. **`POST /chat`** returns **503** quickly if the LLM is not ready (it does **not** block on long HF downloads, avoiding **502** from short gateway timeouts); poll **`GET /readyz`** until **`ok: true`**, then retry. Set **`RAG_BLOCKING_STARTUP_WARMUP=true`** only if your platform allows a long startup without killing the container. **`RAG_AUTO_INGEST_ON_STARTUP`** defaults **off** in code; ship the **CI-built DB** so the image includes **`data/pydantable_vectors.db`**. **`POST /bootstrap`** skips re-ingestion when the index already has rows. Set **`HF_TOKEN`** / **`HUGGING_FACE_HUB_TOKEN`** on the host for Hugging Face. Use **`GET /diag`** (`llm_last_error`) if the LLM fails to load.
