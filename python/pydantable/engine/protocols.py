...
Structural typing: :class:`NativePolarsEngine` implements these without
inheritance. Future backends (for example SQL) would implement the same protocols
where supported and raise :exc:`~pydantable.errors.UnsupportedEngineOperationError`
...
def stub_engine_capabilities() -> EngineCapabilities:
    """Capabilities for :class:`~pydantable.engine.stub.StubExecutionEngine`."""
...
def native_engine_capabilities() -> EngineCapabilities:
    """Capabilities for the current ``pydantable._core`` load, if any."""
