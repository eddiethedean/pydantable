"""Mongo-oriented execution engine: native plans + PyMongo collection roots.

Materialization for sync collection scans uses the optional Mongo plan stack
(installed with ``pip install "pydantable[mongo]"``), which provides plan roots
and columnar helpers. This module implements :class:`MongoPydantableEngine` on
the pydantable side (same pattern as lazy SQL :mod:`pydantable.sql_dataframe`).
"""
...
if NativePolarsEngine is None:
    ...
    class MongoPydantableEngine:  # type: ignore[no-redef]
        ...
        def __init__(self, *_a: Any, **_k: Any) -> None:
            ...
            raise MissingRustExtensionError(
                "MongoPydantableEngine requires pydantable-native (native extension). "
                "Reinstall pydantable or install pydantable-native."
            )

else:

    class MongoPydantableEngine(NativePolarsEngine):  # type: ignore[misc, no-redef]
