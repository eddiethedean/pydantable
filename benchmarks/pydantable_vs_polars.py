"""
Compare pydantable (Rust core + Python API) against native Polars (Python) on
similar workloads.

**Use a release build of the extension** or results skew toward slow debug code:

    .venv/bin/python -m maturin develop --release

Or run ``benchmarks/run_release.sh`` (builds release, then this script + pandas).
...
Run:
    ...
    .venv/bin/python benchmarks/pydantable_vs_polars.py --rows 5000 20000 --rounds 7

By default the **timed region excludes ingest**: building column payloads and
constructing ``DataFrame`` / ``DataFrameModel`` happens once per row count;
only transformations and ``collect()`` are measured. Use ``--with-ingest`` to
include construction in the timer (legacy behavior).

Polars baselines use ``DataFrame.lazy()`` ... ``.collect()`` so the timed work
matches pydantable's lazy pipeline plus materialization (not only eager
``DataFrame`` method chaining).

Ratios > 1 mean pydantable took longer than Polars for that scenario. This is
expected when pydantable adds Pydantic typing, plan serialization, and Python
wrappers on top of the same Polars-backed execution core.
"""
...
def _bench_pipeline_polars(n: int, pl: Any) -> None:
    payload = _build_event_payload(n)
    pdf = pl.DataFrame(payload)
    (
        pdf.lazy()
        .with_columns((pl.col("spend") * 1.1).alias("spend2"))
        .filter(pl.col("spend") > 50.0)
...
def _bench_join_polars(n: int, pl: Any) -> None:
    left_d, right_d = _build_join_payloads(n)
    pl_left = pl.DataFrame(left_d)
    pl_right = pl.DataFrame(right_d)
    pl_left.lazy().join(pl_right.lazy(), on=["id", "ts"], how="inner").collect()
...
def _bench_group_polars(n: int, pl: Any) -> None:
    payload = _build_group_payload(n)
    pdf = pl.DataFrame(payload)
    pdf.lazy().group_by("key").agg(
...
def _run_pipeline_polars_ops(pdf: Any, pl: Any) -> None:
    (
        pdf.lazy()
        .with_columns((pl.col("spend") * 1.1).alias("spend2"))
        .filter(pl.col("spend") > 50.0)
...
def _run_join_polars_ops(pl_left: Any, pl_right: Any) -> None:
    pl_left.lazy().join(pl_right.lazy(), on=["id", "ts"], how="inner").collect()
...
def _run_group_polars_ops(pdf: Any, pl: Any) -> None:
    pdf.lazy().group_by("key").agg(
        pl.col("v").sum().alias("v_sum"),
        pl.col("v").count().alias("v_count"),
    ).collect()
...
def _verify_pipeline_rowcount(n: int, pl: Any) -> None:
    payload = _build_event_payload(n)
    df = EventDF(payload)
    p_rows = len(
    ...
    if p_rows != pl_rows:
        raise RuntimeError(
            f"pipeline row count mismatch: pydantable={p_rows} polars={pl_rows}"
        )
...
def main() -> None:
    ...
    parser.add_argument(
        "--verify",
        action="store_true",
        help="Check pipeline row counts match Polars for the first row count",
    )
    parser.add_argument(
        "--with-ingest",
        action="store_true",
    ...
    if args.verify and args.rows:
        _verify_pipeline_rowcount(args.rows[0], pl)
    ...
    hdr = (
        ...
        f"{'polars_s':>12} {'ratio':>8}"
    )
    print(hdr)
    print("-" * 70)

    for n in args.rows:
        # pipeline_filter_select
        if args.with_ingest:
            t_dt = _mean_time(lambda n=n: _bench_pipeline_pydantable(n), args.rounds)
            t_pl = _mean_time(lambda n=n: _bench_pipeline_polars(n, pl), args.rounds)
        else:
            payload = _build_event_payload(n)
            ...
            t_pl = _mean_time(
                lambda pdf=pdf, pl_mod=pl: _run_pipeline_polars_ops(pdf, pl_mod),
                args.rounds,
            )
    ...
    for n in args.rows:
        ...
        if args.with_ingest:
            ...
            t_pl = _mean_time(lambda n=n: _bench_join_polars(n, pl), args.rounds)
        else:
            left_d, right_d = _build_join_payloads(n)
            left = BenchLeft(left_d)
            right = BenchRight(right_d)
            ...
            t_pl = _mean_time(
                lambda a=pl_left, b=pl_right: _run_join_polars_ops(a, b),
                args.rounds,
            )
        ratio = t_dt / t_pl if t_pl > 0 else float("inf")
        line = (
    ...
    for n in args.rows:
        # groupby_agg_sum_count
        if args.with_ingest:
            t_dt = _mean_time(lambda n=n: _bench_group_pydantable(n), args.rounds)
            t_pl = _mean_time(lambda n=n: _bench_group_polars(n, pl), args.rounds)
        else:
            payload = _build_group_payload(n)
            ...
            t_pl = _mean_time(
                lambda pdf=pdf, pl_mod=pl: _run_group_polars_ops(pdf, pl_mod),
                args.rounds,
            )
        ratio = t_dt / t_pl if t_pl > 0 else float("inf")
        line = (
