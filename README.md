...
**0.13.0** is a **stabilization** release: richer [FastAPI](https://pydantable.readthedocs.io/en/latest/FASTAPI.html) guidance (**`trusted_mode`**, column-shaped JSON bodies, sync materialization vs **0.15.0** async roadmap), explicit **sync-only** I/O in Execution/Performance, and cross-links so **0.12.x** window and trusted-ingest behavior is easier to find.
