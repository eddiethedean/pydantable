...
def test_phase5_row_and_column_inputs_match_under_collect():
    ...
    row_result = (
        row_df.with_columns(age2=row_df.age + 1).filter(row_df.age > 10).select("id", "age2").collect()
    )
    col_result = (
        col_df.with_columns(age2=col_df.age + 1).filter(col_df.age > 10).select("id", "age2").collect()
    )
...
def test_phase5_collect_matches_derived_schema_fields():
    ...
    df2 = df.with_columns(age2=df.age + 1, cond=df.age > 10).select("id", "age2", "cond")
    out = df2.collect()
